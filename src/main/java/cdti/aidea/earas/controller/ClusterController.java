...
public class ClusterController {
    ...
    public ResponseEntity<List<ClusterFormResponseDTO>> getClusterFormData(@PathVariable Long clusterId) {
    ...
    public ResponseEntity<UserClusterSummaryResponse> getUserClusterSummary(@PathVariable Integer userId) {
        try {
            UserClusterSummaryResponse response = clusterService.getUserClusterSummary(userId);
            return ResponseEntity.ok(response);
        } catch (NoSuchElementException e) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body(new UserClusterSummaryResponse(e.getMessage(), 0, 0, 0,0, "CCe Not Available",Collections.emptyList()));
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(new UserClusterSummaryResponse("An internal error occurred", 0, 0, 0, 0,"CCe Not Available",Collections.emptyList()));
        }
    }
//    cluster labels for App
    @PostMapping("/cluster-labels")
    public ResponseEntity<Map<String, Object>> getGroupedClusterFormData(@Valid @RequestBody ClusterIdRequest request) {
    ...
    public ResponseEntity<Map<String, Object>> deleteClusterPlot(@Valid @RequestBody DeleteClusterPlotRequest request) {
...
//    public ResponseEntity<CropReplaceClusterResponse> getCceCrops(@Valid @RequestBody ClusterIdRequest request) {
//        Map<String,Object> response = new LinkedHashMap<>();
//    }
// end app only
...
//    public ResponseEntity<List<VillagesListResponse>> getVillagessByKeyPlot(@PathVariable UUID UserId) {
...
//    }
    ...
    public ResponseEntity<ResbdnoListReponse> getResbdnosByVillageBlock(
        ...
        System.out.println("resvnoStart: " + resvnoStart + ", resvnoEnd: " + resvnoEnd+" "+kpId+" "+villageId+" "+blockCode );

        ResbdnoListReponse response = clusterService.getResbdnoAreaList(kpId, villageId, blockCode, resvnoStart, resvnoEnd);
        return ResponseEntity.ok(response);
    }

    @PostMapping("/next-cluster")
    public CropReplaceClusterResponse getNextCluster(@Valid @RequestBody CropReplaceClusterRequest request) {
...
//        ResbdnoListReponse response = clusterService.getResbdnoAreaList(kpId, villageId, blockCode, resvno);
//        return ResponseEntity.ok(response);
//    }
...
//    }
    ...
    public ResponseEntity<ClusterPlotAreaRes> getPlotDetails(
            @PathVariable UUID kpId,
            @RequestParam Integer resvno,
            @RequestParam String resbdno) {
    ...
    public ResponseEntity<?> saveClusterForm(@RequestBody SaveClusterRequestDTO request) {
        try {
            ...
            return ResponseEntity.ok(Collections.singletonMap("message", "Cluster form saved successfully."));
...
//        try {
//            Map<String, Object> newPlot = keyPlots_Service.rejectAndReplaceKeyplot(keyPlotId, request);
//            return ResponseEntity.ok(newPlot);
//        } catch (EntityNotFoundException e) {
//            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(e.getMessage());
//        } catch (RuntimeException e) {
//            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(e.getMessage()); // Or INTERNAL_SERVER_ERROR based on the type of error
//        } catch (Exception e) {
//            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
