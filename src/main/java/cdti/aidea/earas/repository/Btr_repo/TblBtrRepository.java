...
public interface TblBtrRepository extends JpaRepository<TblBtrData, Long> {
    ...
    @Query("SELECT m FROM TblBtrData m WHERE m.lsgcode IN :lsgcodes AND " +
            ...
            "m.ltype LIKE %:filter%)")
    Page<TblBtrData> findByLsgcodeInAndFilter(@Param("lsgcodes") List<Integer> lsgcodes, @Param("filter") String filter, Pageable pageable);
}
