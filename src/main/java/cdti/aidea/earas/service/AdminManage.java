    ...
    if (request.getId() != null) {
      log =
          repository
              ...
              .orElseThrow(
                  () -> new RuntimeException("Record not found with ID: " + request.getId()));
      ...
      if (request.getAddedBy() == null || request.getAddedBy().equals(log.getEditPermitter())) {
        throw new IllegalStateException(
            "Edit not allowed by admin. Another user must perform the edit.");
      }
      ...
      if (exists) {
        throw new IllegalStateException(
            "A record already exists for the current agri year: " + agriStart);
      }
  ...
  public ClusterLimitLog saveOrUpdateClusterLimit(ClusterLimitRequest request) {
    ...
    if (request.getId() != null) {
      ...
      if (clusterLimitLogRepository.existsByAgriStartAndEndYear(startYear, endYear)) {
        throw new IllegalStateException(
            "A record already exists for agri year " + startYear + " - " + endYear);
      }
