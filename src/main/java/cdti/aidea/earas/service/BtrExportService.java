...
//            .findAllByUserId(userId).get(
//            .orElseThrow(() -> new RuntimeException("User not found"));
//
//    var zoneRevenueList =
//        tblZoneRevenueVillageMappingRepository.findByZone(user.getTblMasterZone().getZoneId());
//
...
//  }
  ...
  //                .collect(Collectors.toMap(v -> String.valueOf(v.getVillageCodeApi()),
  // TblMasterVillage::getVillageNameMal));
  //
  //        List<String> lbCodes = allData.stream().map(TblBtrData::getLbcode).distinct().toList();
  //        Map<Integer, String> localBodyMap =
  // localBodyRepository.findAllByCodeApiIn(lbCodes).stream()
  //                .collect(Collectors.toMap(TblLocalBody::getLocalbodyId,
  // TblLocalBody::getLocalbodyNameMal));
  //
  //        Map<String, String> landTypeMap =
