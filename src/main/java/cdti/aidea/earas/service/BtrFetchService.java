...
public class BtrFetchService {
  ...
  @Autowired private TblBtrDataRepository tblBtrDataRepository;

  @Autowired private TblMasterVillageRepository tblMasterVillageRepository;

  @Autowired private LocalBodyRepository localBodyRepository;

  @Autowired private TblZoneRevenueVillageMappingRepository zoneRevenueVillageMappingRepository;
  ...
  public Optional<BtrDataResponse> getBtrDataById(Long id) {
    try {
      ...
      if (btrData.getLsgcode() != null) {
        Optional<TblMasterVillage> villageOpt =
            tblMasterVillageRepository.findByLsgCode(btrData.getLsgcode());
      ...
      if (btrData.getLbcode() != null) {
        Optional<TblLocalBody> localBodyOpt =
            localBodyRepository.findByCodeApi(btrData.getLbcode());
      ...
      return btrDataList.stream()
          .map(
              data -> {
                ...
                Integer zoneId =
                    village.map(v -> villageToZoneMap.get(v.getVillageId())).orElse(null);
                response.setZoneId(zoneId);
  ...
  public String getZoneSummary(Integer zoneId) {
    try {
      ...
      long uniqueLocalBodies =
          data.stream().map(BtrDataResponse::getLbcode).filter(Objects::nonNull).distinct().count();
      long uniqueVillages =
          data.stream()
      ...
      return String.format(
          "Zone %d: %d BTR records across %d local bodies and %d villages",
      ...
      return data.stream()
          .collect(
              Collectors.groupingBy(
                  response ->
                      response.getLtype() != null && !response.getLtype().trim().isEmpty()
                          ? response.getLtype()
                          : "Unknown",
                  Collectors.counting()));
  ...
  private List<BtrDataResponse> convertToResponseDTOs(
      List<TblBtrData> btrDataList, List<TblMasterVillage> villageList, Integer zoneId) {
    ...
    return btrDataList.stream()
        ...
        .sorted(
            Comparator.comparing(
                    (BtrDataResponse r) -> r.getLocalBodyName() != null ? r.getLocalBodyName() : "")
  ...
  private Integer findZoneByVillageId(Integer villageId) {
    ...
    try {
      List<TblZoneRevenueVillageMapping> mappings =
          zoneRevenueVillageMappingRepository.findAll().stream()
              .filter(
                  mapping ->
                      mapping.getRevenueVillage() != null
                          && mapping.getRevenueVillage().equals(villageId)
                          && mapping.getIsValid() != null
                          && mapping.getIsValid())
              .collect(Collectors.toList());
