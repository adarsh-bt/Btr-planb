...
// 👉 Add this
    ...
    public List<Map<String, Object>> getSystematicSample(List<TblBtrDataOld> plots, int clusters, int interval, int randomStart,Map<String, String> localBodyNameMap,AtomicInteger globalSlNo) {
        List<Map<String, Object>> sample = new ArrayList<>();

        if (plots.isEmpty() || clusters == 0 || interval <= 0) return sample;

        // Sort plots by Sy. No (or area or any consistent key)
        plots.sort(Comparator.comparing(TblBtrDataOld::getId));  // Sort by ID or any field that makes sense
...
//        int randomStarts = random.nextInt(plots.size());
        ...
        for (int i = 0; i < clusters; i++) {
            ...
            TblBtrDataOld plot = plots.get(index-1);
...
//    }
    ...
    public Map<String, Object> rejectAndReplaceKeyplot(Long keyPlotId, KeyPlotRejectRequest request) {
        ...
        TblBtrDataOld btrData = tblBtrDataOldRepository.findById(keyPlotId)
                .orElseThrow(() -> new EntityNotFoundException("BTR data not found with ID: " + keyPlotId));

        KeyPlots rejectedKeyPlot = keyPlotsRepository.findByBtrDataOld(btrData)
        ...
        List<TblBtrDataOld> replacements = tblBtrDataOldRepository
        ...
        TblBtrDataOld newPlot = replacements.get(new Random().nextInt(replacements.size()));

        // 7. Create new keyplot
        KeyPlots newKeyPlot = new KeyPlots();
        newKeyPlot.setBtrDataOld(newPlot);
        ...
        if (clusterOpt.isPresent()) {
            ...
            ClusterFormData clusterFormData = new ClusterFormData();
            clusterFormData.setPlot(newKeyPlot.getBtrDataOld());
            clusterFormData.setPlotLabel("K");
            ...
            clusterFormData.setEnumeratedArea(newKeyPlot.getBtrDataOld().getArea());
        ...
        newPlotMap.put("Sy. No", newKeyPlot.getBtrDataOld().getResvno() + "/" + newKeyPlot.getBtrDataOld().getResbdno());
        newPlotMap.put("panchayth", newKeyPlot.getLocalbody().getLocalbodyNameEn());
        newPlotMap.put("Area (Cents)", newKeyPlot.getBtrDataOld().getArea());
        newPlotMap.put("Village/Block", newKeyPlot.getBtrDataOld().getBcode());
    ...
    public KeyPlotDetailsResponse getKeyPlotDetails(UUID plotId) {
        // 1) Load KeyPlot and related basics
        KeyPlots keyPlot = keyPlotsRepository.findById(plotId)
                .orElseThrow(() -> new NoSuchElementException("Plot ID not found: " + plotId));

        TblBtrDataOld plot = keyPlot.getBtrDataOld();
        ...
        String panchayath = localBodyRepository.findByCodeApi(lbcode)
                ...
                .orElse(lbcode); // fallback

        String landType = keyPlot.getLandType();

        Optional<TblMasterVillage> villageOpt = tblMasterVillageRepository.findByLsgCode(plot.getLsgcode());
        String villageName = villageOpt.map(TblMasterVillage::getVillageNameEn).orElse(null);
        Integer villageId = villageOpt.map(TblMasterVillage::getVillageId).orElse(null);

        // 2) Fetch related SidePlotDTOs
        List<SidePlotDTO> sidePlots = fetchSidePlotsForKeyPlot(keyPlot);

        // 3) Fetch the single current-active cluster limits
        ...
        // 4) Build and return response including current active cluster limits
