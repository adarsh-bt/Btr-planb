...
public class KeyPlots_Service {
  ...
  //    private void saveSamples(List<Map<String, Object>> samples, int interval, int zone_id,
  // String landType,
  //                             TblLocalBody localbody, UUID userId, AtomicInteger clusterCounter)
  // {
  //
  //        UserZoneAssignment zone =
  // userZoneAssignmentRepositoty.findByTblMasterZone_ZoneId(zone_id)
  //                .orElseThrow(() -> new EntityNotFoundException("BTR data not found: " +
  // zone_id));
  //
  ...
  //    public Map<String, Object> rejectAndReplaceKeyplot(Long keyPlotId, KeyPlotRejectRequest
  // request) {
  //
  //        // 1. Find the keyplot to be rejected
  //        TblBtrData btrData = tblBtrDataRepository.findById(keyPlotId)
  //                .orElseThrow(() -> new EntityNotFoundException("BTR data not found with ID: " +
  // keyPlotId));
  //
  //        KeyPlots rejectedKeyPlot = keyPlotsRepository.findByBtrData(btrData)
  //                .orElseThrow(() -> new EntityNotFoundException("Keyplot not found for BTR ID: "
  // + keyPlotId));
  //
  ...
  // userZoneAssignmentRepositoty.findByTblMasterZone_ZoneId(rejectedKeyPlot.getZone().getZoneId());
  //        if (userzone.isEmpty()) {
  ...
  // clusterMasterRepository.findByKeyPlotId(rejectedKeyPlot.getId());
  ...
  //            AvailableCcePlotRejectionRequest rejectionRequest = new
  // AvailableCcePlotRejectionRequest();
  ...
  //
  //            try {
  //                ResponseEntity<Response> rejectionResponse =
  // formEntryClient.availableCcePlotRejection(rejectionRequest);
  //
  //                if (!rejectionResponse.getStatusCode().equals(HttpStatus.CREATED)) {
  //                    String message = (rejectionResponse.getBody() != null) ?
  // rejectionResponse.getBody().getMessage() : "Unknown error";
  //
  //                    if (message != null && message.contains("Active plots with given oldPlotId
  // and oldClusterId not found")) {
  //                        // Log it, but don't block execution
  //                        System.out.println("CCE system had no matching data. Skipping sync.
  // Message: " + message);
  //                    } else {
  ...
  //
  //        newPlotMap.put("Sy. No", newKeyPlot.getBtrData().getResvno() + "/" +
  // newKeyPlot.getBtrData().getResbdno());
  ...
  private List<SidePlotDTO> fetchSidePlotsForKeyPlot(KeyPlots keyPlot) {
    Optional<ClusterMaster> clusterOpt =
        clusterMasterRepository.findTopByKeyPlotOrderByCreatedAtDesc(keyPlot);
    ...
    Map<Integer, String> lsgcodeToVillageNameMap =
        villages.stream()
            .collect(
