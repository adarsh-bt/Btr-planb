...
public class TblBtrDataService {
    ...
    public Map<String, Object> saveAllData(List<TblBtrDataDTO> dtoList) {
        ...
        for (TblBtrDataDTO dto : dtoList) {
            ...
            try {
                ...
                allErrors.add(
                        new ValidationErrorResponse(dto.getResvno(), dto.getResbdno(), ex.getMessage()));
            }
        }
    ...
    private void validateZoneMapping(TblBtrDataDTO dto) {
        ...
        boolean exists =
                villages.stream().anyMatch(v -> v.getLsgCode().equals(dto.getLsgcode()));

        if (!exists) {
            throw new RuntimeException(
                    "Zone mismatch! Provided lsgCode=" + dto.getLsgcode() + " does not belong to zoneId=" + zoneId);
        }
    }
