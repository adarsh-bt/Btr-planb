  ...
  public VillageLocalBodyMap linkVillageToLocalbody(Integer villageId, Integer localbodyId) {
    TblMasterVillage village =
        villageRepository
            .findById(villageId)
            .orElseThrow(
                () -> new EntityNotFoundException("Village not found with id " + villageId));

    TblLocalBody localbody =
        localBodyRepository
            .findById(localbodyId)
            .orElseThrow(
                () -> new EntityNotFoundException("Localbody not found with id " + localbodyId));

    VillageLocalBodyMap map =
        VillageLocalBodyMap.builder().village(village).localBody(localbody).isActive(true).build();
