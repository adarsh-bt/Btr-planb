...
public class ZoneMappingService {
    ...
    public List<LocalbodyDto> getLocalbodiesByZone(Integer zoneId, String lang) {
        List<TblZoneLocalbodyMapping> mappings = zoneLbRepo.findAllByZoneAndIsValid(zoneId, Boolean.TRUE);

        if (mappings.isEmpty()) return List.of();

        List<Integer> lbIds = mappings.stream()
    ...
    public List<RevenueTalukDto> getRevenueTaluksByZone(Integer zoneId, String lang) {
        ...
        return mappings.stream()
                ...
                .map(m -> new RevenueTalukDto(
                        m.getRevenueTaluk(),
                        m.getRevTalukNameEn() // only English
                ))
                .distinct()
                .sorted(Comparator.comparing(RevenueTalukDto::getRevenueTalukName, String.CASE_INSENSITIVE_ORDER))
                .toList();
    }
    ...
    public List<RevenueVillageDto> getRevenueVillagesByZone(Integer zoneId, String lang) {
        ...
        List<TblZoneRevenueVillageMapping> mappings = zoneRevenueVillageMappingRepository.findByZone(zoneId);

        List<Integer> villageIds = mappings.stream()
                .filter(m -> Boolean.TRUE.equals(m.getIsValid()))
        ...
        List<TblMasterVillageBlock> blocks = tblMasterVillageBlockRepository.findByVillageIdIn(villageIds);

        // Map villageId -> list of blockCodes
        Map<Integer, List<String>> villageBlockMap = blocks.stream()
