...
public class Zone_Service {
    ...
    public List<ZoneListResponse> UserZonesByType(String type, Integer idValue) {
        try {
            ...
            List<TblMasterZone> availableZones = zones.stream()
                    .filter(zone -> !assignedZoneIds.contains(zone.getZoneId().longValue()))
                    .collect(Collectors.toList());

            // Map the available zones to the response DTO
            List<ZoneListResponse> zoneList = availableZones.stream()
                    .map(zone -> new ZoneListResponse(zone.getZoneId(), zone.getZoneCode(), zone.getZoneNameEn(), zone.getZoneNameMal()))
                    .collect(Collectors.toList());
    ...
    public UserZoneAssignment ZoneAssignedService(ZoneAssignedRequset request) {
        ...
        try {
            ...
            TblMasterZone tblMasterZone = tblMasterZoneRepository.findById(request.getZoneId())
                    .orElseThrow(() -> new IllegalArgumentException("Zone does not exist"));

            // 2. Check if zone is already assigned to another user
            Optional<UserZoneAssignment> existingZoneAssignment = userZoneAssignmentRepositoty
                    .findByTblMasterZone_ZoneId(request.getZoneId());
            ...
            Optional<UserZoneAssignment> existingUserAssignment = userZoneAssignmentRepositoty
                    .findByUserId(request.getUser_id());
...
//        List<TblBtrData> allData = tblBtrRepository.findAllByLsgcodeIn(lsgcodes);
        ...
        List<String> LbcodeList = allData.stream().map(TblBtrDataOld::getLbcode).distinct().collect(Collectors.toList());
...
//        Pageable pageable = PageRequest.of(page, size);
        ...
        Pageable pageable = PageRequest.of(
                page,
                size,
                Sort.by("lbcode").and(                // 1. Localbody code
                        Sort.by("lsgcode").and(           // 2. Village
                                Sort.by("bcode").and(         // 3. Block
                                        Sort.by("resvno").and(    // 4. Survey No part 1
                                                Sort.by("resbdno").and( // 4. Survey No part 2
        ...
        if (filter == null || filter.isEmpty()) {
            ...
            pageResult = tblBtrDataOldRepository.findByLsgcodeInWithNamesFilter(lsgcodes, filter, pageable);
        }
...
//            double nhect = data.getNhect() != null ? data.getNhect() : 0;
        ...
        double totalWetAreas = new BigDecimal(totalWetArea).setScale(2, RoundingMode.HALF_UP).doubleValue();
        double totalDryAreas = new BigDecimal(totalDryArea).setScale(2, RoundingMode.HALF_UP).doubleValue();
        double totalConvertedAreas = new BigDecimal(totalConvertedArea).setScale(2, RoundingMode.HALF_UP).doubleValue();
...
//                    double totalCent = (myTable.getNhect() != null ? myTable.getNhect() : 0) * 247.13
...
//                    double totalCent = myTable.getArea()
                    ...
                    BigDecimal bd = new BigDecimal(myTable.getArea())
...
//        System.out.println("land Type  " + landType);
//
//        List<String> LbcodeList = allData.stream().map(TblBtrData::getLbcode).distinct().collect(Collectors.toList());
//        System.out.println("Lbcode " + LbcodeList);
//        List<TblLocalBody> localBodies_full = localBodyRepository.findAllByCodeApiIn(LbcodeList);
//
...
//
//        double totalArea = pageResult.getContent().stream()
//                .mapToDouble(TblBtrData::getNsqm) // Assuming nsqm is the field you want to sum up
//                .sum();
//
//        // Prepare a map for village codes and names
//        Map<String, String> villageNameMap = villageList.stream()
//                .collect(Collectors.toMap(TblMasterVillage::getVillageCodeApi, TblMasterVillage::getVillageNameMal));
//
//        // Create a map to fetch LocalBody details based on lbcode
//        Map<String, String> localBodyNameMap = new HashMap<>();
//        List<String> lbCodes = pageResult.getContent().stream()
